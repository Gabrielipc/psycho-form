...
class SearchControllerTest {
    ...
    void searchDevuelveParticipantesYSesionesQueCoinciden() {
        ...
        when(participantes.searchByTerm("ana", PageRequest.of(0, 8))).thenReturn(List.of(participante));
        when(sesiones.searchByTerm("ana", PageRequest.of(0, 8))).thenReturn(List.of(sesion));

        @SuppressWarnings("unchecked")
        List<SearchResult> results = (List<SearchResult>) controller.search("ana").data();

        assertThat(results).extracting(SearchResult::type)
                .containsExactly("participante", "sesion");
        assertThat(results).extracting(SearchResult::path)
